...
const unreadableEntries = ref(0)
const emptySelectedFolder = ref(false)
...
async function selectDesktopFolder(): Promise<void> {
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    const localFailures: ItemImportacaoLote[] = []
    ...
    for (const [index, selected] of selectedFiles.value.entries()) {
      try {
        ...
        localFailures.push({
          ...
          caminho_relativo: selected.relativePath,
          status: 'falha',
          matriz_id: null,
          motivo_falha: 'O arquivo não pôde ser lido no computador.',
        })
      }
    }
    ...
    if (!readableFiles.length) {
      throw new Error('Nenhum arquivo selecionado pôde ser lido.')
    }

    const apiReport = await importService.batch(
      readableFiles,
      {
        caminhos_relativos: readablePaths,
    ...
    report.value = localFailures.length
      ? {
          ...apiReport,
          total_arquivos: apiReport.total_arquivos + localFailures.length,
          arquivos_com_falha: apiReport.arquivos_com_falha + localFailures.length,
          itens: [...apiReport.itens, ...localFailures],
        }
      : apiReport
  } catch (requestError) {
...
<template>
  <section class="mx-auto max-w-5xl">
    ...
    >
      ...
      >
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-3">
          <button
            ...
            :disabled="sending || scanningFolder"
            @click="selectDesktopFolder"
          >
            <LoadingSpinner v-if="scanningFolder" />
            <FolderOpen v-else :size="18" />
            {{ scanningFolder ? 'Lendo pasta…' : 'Selecionar pasta do computador' }}
          </button>
      ...
      <div
        v-if="emptySelectedFolder"
        class="mt-5 flex items-start gap-2 rounded-xl bg-amber-50 p-4 text-sm text-amber-800"
        role="status"
      >
        <AlertCircle :size="19" class="shrink-0" />
        A pasta “{{ selectedFolderName }}” não contém arquivos .PES.
      </div>

      <div
        v-if="unreadableEntries"
        class="mt-5 flex items-start gap-2 rounded-xl bg-amber-50 p-4 text-sm text-amber-800"
        role="status"
      >
        <AlertCircle :size="19" class="shrink-0" />
        {{ unreadableEntries }}
        {{ unreadableEntries === 1 ? 'item não pôde ser lido' : 'itens não puderam ser lidos' }}
        e foi ignorado. Os demais arquivos continuam disponíveis.
      </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-line p-4">
          <div>
            ...
            <p class="mt-0.5 text-xs text-muted">
              ...
              <span v-if="selectedFolderName"> · pasta {{ selectedFolderName }}</span>
      ...
      <div v-if="sending" class="mt-6" aria-live="polite">
        <div class="mb-2 flex justify-between text-sm text-muted">
          <span>
            {{ preparingFiles
              ? 'Lendo arquivos da pasta selecionada…'
              : uploadProgress < 100
                ? 'Enviando arquivos…'
                : 'Processando matrizes e gerando previews…' }}
          </span>
          <span v-if="!preparingFiles">{{ uploadProgress }}%</span>
        </div>
